...
class AddBook extends Component {
    ...
    render() {
        const {bookName, authorName,isBookEmpty, isAuthorEmpty, isSubmited} = this.state
        if(isSubmited) {
            return <Navigate to='/' />
        }
        return (
            <div className="add-book-form-container">
                <form onSubmit={this.onSubmitForm} className="add-book-form-card">
                    ...
                    <div className='add-book-page-input-container'>
                        <label htmlFor='book' className='add-book-input-label'>Book Name</label>
                        <input type='text' className='add-book-input-feild' placeholder='Enter book name' id='book' value={bookName} onChange={this.onChangeBookName} onBlur={this.onBlurBookName} />
                        {isBookEmpty && <p className='add-book-input-err-msg'>*Required</p>}
                    </div>
                    <div className='add-book-page-input-container'>
                        <label htmlFor='author' className='add-book-input-label'>Author Name</label>
                        <input type='text' className='add-book-input-feild' placeholder='Author book name' id='author' value={authorName} onChange={this.onChangeAuthorName} onBlur={this.onBlurAuthorName}/>
                        {isAuthorEmpty && <p className='add-book-input-err-msg'>*Required</p>}
                    </div>
                    <div className='add-book-page-button-card'>
                        <button className='add-book-page-button' type='submit'>Add</button>
                    </div>
                </form>
            </div>
        )
